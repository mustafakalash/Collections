...
public class PaletteWidget
{
    ...
    private static readonly Dictionary<int, List<StainAdapter>> StainsByShade = Services.DataProvider.SupportedStains.GroupBy(s => (int)s.Stain.Shade).ToDictionary(s => s.Key, s => s.ToList());
    private static readonly StainAdapter EmptyStain = ExcelCache<StainAdapter>.GetSheet().GetRow(0).Value;
    ...
    public void DrawPalette()
    {
        ...
        foreach (var (shade, stainList) in StainsByShade)
        {
            ...
            for (var j = 0; j < stainList.Count; j++)
            {
                ...
                if (ActiveStain.Stain.Equals(stain.Stain))
                {
                    ...
                    ImGui.ColorButton(stain.Stain.Name.ToString(), ColorsPalette.YELLOW, ImGuiColorEditFlags.NoTooltip, stainButtonRectSize);
                    ImGui.SetCursorPos(origPos);
                }

                // Draw button
                if (ImGui.ColorButton(stain.Stain.Name.ToString(), colorVec, ImGuiColorEditFlags.NoSidePreview | ImGuiColorEditFlags.NoSmallPreview | ImGuiColorEditFlags.NoTooltip, stainButtonSize))
                {
                    ActiveStain = stain;
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text(stain.Stain.Name.ToString());
                    ImGui.EndTooltip();
                }
    ...
    private void DrawColorPicker()
    {
        ...
        if (ImGui.ColorPicker3("##color-picker", ref pickedColor, ImGuiColorEditFlags.NoSidePreview | ImGuiColorEditFlags.NoTooltip | ImGuiColorEditFlags.NoInputs | ImGuiColorEditFlags.NoOptions))
        {
            ...
            if (!ActiveStain.Stain.Equals( newStain.Stain))
            {
